...
const { data: existingExpense } = await useFetch(`/api/expenses/${expenseId}`);
const { data: users } = await useFetch("/api/users");

// Initialize form state with existing expense data
const shareInvolvementsFromExisting = existingExpense.value?.involvements.filter(
  (inv) => inv.type === "share"
) || [];

const formState = ref<Partial<ExpenseSchema>>({
  totalAmount: existingExpense.value?.totalAmount || 0,
  emoji: existingExpense.value?.emoji || "",
  title: existingExpense.value?.title || "",
  userId: existingExpense.value?.userId || undefined,
  participantIds: shareInvolvementsFromExisting.map((inv) => inv.userId),
  splitType: existingExpense.value?.splitType || "equal",
  paidAt: existingExpense.value?.paidAt || today(getLocalTimeZone()).toString(),
});
...
// Initialize split ratio from existing data
const initialSplitRatio = new Map(
  shareInvolvementsFromExisting
    .filter((inv) => inv.shareRatio !== null)
    .map((inv) => [inv.userId, inv.shareRatio || 1])
);

// If no share ratios exist, default to 1 for all participants
const splitRatio = ref(
  initialSplitRatio.size > 0
    ? initialSplitRatio
...
// Initialize manual split from existing data
const initialManualSplit = new Map(
  shareInvolvementsFromExisting.map((inv) => [inv.userId, inv.amount])
);

const manualSplit = ref(
  initialManualSplit.size > 0
    ? initialManualSplit
...
const haveManuallySetSplit = ref(
  new Map(
    formState.value?.participantIds?.map((id) => [
      id,
      existingExpense.value?.splitType === "manual",
    ])
  )
);
