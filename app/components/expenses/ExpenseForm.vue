...
const props = withDefaults(defineProps<ExpenseFormProps>(), {
  submitButtonText: "💾 追加",
  isEdit: false,
});
...
const formState = ref<Partial<ExpenseSchema>>({
  ...
  participantIds: props.initialData?.participantIds || props.users?.map((u) => u.id) || [],
  splitType: props.initialData?.splitType || "equal",
  paidAt: props.initialData?.paidAt || today(getLocalTimeZone()).toString(),
});
...
// Initialize split ratio from existing data if in edit mode
const initializeSplitRatio = () => {
  if (props.isEdit && props.initialData?.involvements && props.initialData.splitType === "ratio") {
    const ratioMap = new Map();
    props.initialData.involvements.forEach((inv: any) => {
      if (inv.type === "share" && inv.shareRatio != null) {
        ratioMap.set(inv.userId, inv.shareRatio);
      }
    });
    return ratioMap;
  }
  return new Map(formState.value?.participantIds?.map((id) => [id, 1]));
};

const splitRatio = ref(initializeSplitRatio());
...
// Initialize manual split from existing data if in edit mode
const initializeManualSplit = () => {
  if (props.isEdit && props.initialData?.involvements && props.initialData.splitType === "manual") {
    const manualMap = new Map();
    props.initialData.involvements.forEach((inv: any) => {
      if (inv.type === "share") {
        manualMap.set(inv.userId, inv.amount);
      }
    });
    return manualMap;
  }
  return new Map(formState.value?.participantIds?.map((id) => [id, 0]));
};

const manualSplit = ref(initializeManualSplit());

const haveManuallySetSplit = ref(
  new Map(
    formState.value?.participantIds?.map((id) => [id, props.isEdit && props.initialData?.splitType === "manual"])
  )
);
  ...
  if (formState.value.splitType === "equal") {
    ...
    return [
      ...
      })) || []),
      {
        ...
        user: props.users?.find((user) => user.id === formState.value.userId),
        type: "payment",
...
const handleFormSubmit = async (event: FormSubmitEvent<ExpenseSchema>) => {
  ...
  if (props.isEdit && props.expenseId) {
    // Update existing expense
    const response = await $fetch(`/api/expenses/${props.expenseId}`, {
      method: "PUT",
      body: payload,
    });
